...
<template>
  <div id="store" class="shadow_container">
    ...
    <el-form ref="find_form" :model="find_form" label-width="80px">
      ...
      <el-form-item label="角色组名称" label-width="100px">
        <el-input v-model="find_form.data.roleGrName" placeholder="请输入角色组名称"></el-input>
      </el-form-item>
    ...
    <el-table :data="store_list" border style="width: 100%"  @selection-change="handleSelectionChange">
      ...
      <el-table-column fixed prop="roleId" label="角色ID" width="80"></el-table-column>
      <el-table-column prop="roleGrName" label="角色组名称" width="120"></el-table-column>
    ...
    <el-dialog title="添加类别" :visible.sync="dialogFormVisibleOfAdd" width="40%">
      ...
      <el-form ref="add_form" :model="add_form" label-width="120px">
        <el-form-item label="角色组名称">
          <el-input v-model="add_form.roleGrName" placeholder="请输入角色组名称" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="修改类别" :visible.sync="dialogFormVisibleOfEdit" width="40%">
      ...
      <el-form ref="edit_form" :model="edit_form" label-width="120px">
        <el-form-item label="角色组名称">
          <el-input v-model="edit_form.roleGrName" placeholder="请输入角色组名称" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  mounted() {
    ...
    getDataList("m1", "Rolegroup", get_form, "store_list", this);
  },
  methods: {
    queryData() {
    ...
    user_add() {
      ...
      putcreate("m1", "Rolegroup",add_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    // 修改当前
    user_update() {
      patchedit("m1", "Rolegroup",this.edit_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    user_delete(val) {
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deleteuser("m1", "Rolegroup",del).then((res) => {
          console.log(res);
          if (res != null) {
    ...
    delete_list(){
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deletelist("m1", "Rolegroup",dellist).then((res) => {
          console.log(res);
          if (res != null) {
            this.$message.success("删除成功");
            this.resetForm();
