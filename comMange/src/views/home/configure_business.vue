...
<template>
  <div id="store" class="shadow_container">
    <div class="pageTitle">经营类别</div>
    <!-- 查询条件 -->
    <el-form ref="find_form" :model="find_form" label-width="80px">
      <el-form-item label="自身ID" label-width="100px">
        <el-input v-model="find_form.data.bcId" placeholder="请输入自身ID"></el-input>
      </el-form-item>
      <el-form-item label="父级ID" label-width="100px">
        <el-input v-model="find_form.data.pbcId" placeholder="请输入父级ID"></el-input>
      </el-form-item>
      <el-form-item label="经营类别名" label-width="100px">
        <el-input v-model="find_form.data.bcName" placeholder="请输入经营类别名"></el-input>
      </el-form-item>
    ...
    <el-table :data="store_list" border style="width: 100%"  @selection-change="handleSelectionChange">
      ...
      <el-table-column fixed prop="bcId" label="自身id" width="80"></el-table-column>
      <el-table-column prop="pbcId" label="父级id" width="120"></el-table-column>
      <el-table-column prop="bcName" label="经营类别名" width="120"></el-table-column>
    ...
    <el-dialog title="添加类别" :visible.sync="dialogFormVisibleOfAdd" width="40%">
      ...
      <el-form ref="add_form" :model="add_form" label-width="120px">
        <el-form-item label="经营类别名">
          <el-input
            v-model="add_form.bcName"
            placeholder="请输入经营类别名"
          ></el-input>
        </el-form-item>
        <el-form-item label="父级id">
          <el-input v-model="add_form.pbcId" placeholder="请输入父级id" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="修改类别" :visible.sync="dialogFormVisibleOfEdit" width="40%">
      ...
      <el-form ref="edit_form" :model="edit_form" label-width="120px">
        <el-form-item label="经营类别名">
          <el-input
            v-model="edit_form.bcName"
            placeholder="请输入经营类别名"
          ></el-input>
        </el-form-item>
        <el-form-item label="父级id">
          <el-input v-model="edit_form.pbcId" placeholder="请输入父级id" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  mounted() {
    ...
    getDataList("c1", "Businesscategory", get_form, "store_list", this);
  },
  methods: {
    queryData() {
    ...
    user_add() {
      ...
      putcreate("c1","Businesscategory",add_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    // 修改当前
    user_update() {
      patchedit("c1","Businesscategory",this.edit_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    user_delete(val) {
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deleteuser("c1","Businesscategory",del).then((res) => {
          console.log(res);
          if (res != null) {
    ...
    delete_list(){
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deletelist("c1","Businesscategory",dellist).then((res) => {
          console.log(res);
          if (res != null) {
            this.$message.success("删除成功");
            this.resetForm();
