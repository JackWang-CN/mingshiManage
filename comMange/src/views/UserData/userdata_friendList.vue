...
<template>
  <div id="store" class="shadow_container">
    ...
    <el-form ref="find_form" :model="find_form" label-width="80px">
      ...
      <el-form-item label="好友ID" label-width="100px">
        <el-input v-model="find_form.data.friendId" placeholder="请输入好友ID"></el-input>
      </el-form-item>
    ...
    <el-table :data="store_list" border style="width: 100%"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="friendId" label="好友ID" width="120"></el-table-column>
    ...
    <el-dialog title="添加类别" :visible.sync="dialogFormVisibleOfAdd" width="40%">
      ...
      <el-form ref="add_form" :model="add_form" label-width="120px">
        ...
        <el-form-item label="好友ID">
          <el-input v-model="add_form.friendId" placeholder="请输入好友ID" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="是否删除">          
    ...
    <el-dialog title="修改类别" :visible.sync="dialogFormVisibleOfEdit" width="40%">
      ...
      <el-form ref="edit_form" :model="edit_form" label-width="120px">
        <el-form-item label="好友ID">
          <el-input v-model="edit_form.friendId" placeholder="请输入好友ID" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="是否删除">          
...
export default {
  ...
  mounted() {
    ...
    getDataList("u1", "Friendlist", get_form, "store_list", this);
  },
  methods: {
    queryData() {
    ...
    user_add() {
      ...
      putcreate("u1", "Friendlist",add_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    // 修改当前
    user_update() {
      patchedit("u1", "Friendlist",this.edit_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    user_delete(val) {
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deleteuser("u1", "Friendlist",del).then((res) => {
          console.log(res);
          if (res != null) {
    ...
    delete_list(){
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deletelist("u1", "Friendlist",dellist).then((res) => {
          console.log(res);
          if (res != null) {
            this.$message.success("删除成功");
            this.resetForm();
