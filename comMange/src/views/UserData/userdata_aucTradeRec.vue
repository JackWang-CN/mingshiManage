  ...
  <div id="store" class="shadow_container">
    ...
    <el-form ref="find_form" :model="find_form" label-width="80px">
      <el-form-item label="交易单号（数据ID）" label-width="100px">
        <el-input v-model="find_form.data.tradeId" placeholder="请输入交易单号（数据ID）"></el-input>
      </el-form-item>
      ...
      <el-form-item label="交易人" label-width="100px">
        <el-input v-model="find_form.data.trader" placeholder="请输入交易人"></el-input>
      </el-form-item>
    ...
    <el-table :data="store_list" border style="width: 100%"  @selection-change="handleSelectionChange">
      ...
      <el-table-column fixed prop="tradeId" label="交易单号（数据ID）" width="80"></el-table-column>
      <el-table-column prop="resId" label="资源id" width="120"></el-table-column>
      <el-table-column fixed prop="trader" label="交易人" width="80"></el-table-column>
    ...
    <el-dialog title="添加类别" :visible.sync="dialogFormVisibleOfAdd" width="40%">
      ...
      <el-form ref="add_form" :model="add_form" label-width="120px">
        <el-form-item label="交易单号（数据ID）">
          <el-input v-model="add_form.tradeId" placeholder="请输入交易单号（数据ID）" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="资源id">
          <el-input v-model="add_form.resId" placeholder="请输入资源id" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="交易人">
          <el-input v-model="add_form.trader" placeholder="请输入交易人" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="修改类别" :visible.sync="dialogFormVisibleOfEdit" width="40%">
      ...
      <el-form ref="edit_form" :model="edit_form" label-width="120px">
        ...
        <el-form-item label="交易人">
          <el-input v-model="edit_form.trader" placeholder="请输入交易人" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    user_add() {
      ...
      putcreate("u1", "Auctraderec",add_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    // 修改当前
    user_update() {
      patchedit("u1", "Auctraderec",this.edit_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    user_delete(val) {
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deleteuser("u1", "Auctraderec",del).then((res) => {
          console.log(res);
          if (res != null) {
    ...
    delete_list(){
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deletelist("u1", "Auctraderec",dellist).then((res) => {
          console.log(res);
          if (res != null) {
            this.$message.success("删除成功");
            this.resetForm();
