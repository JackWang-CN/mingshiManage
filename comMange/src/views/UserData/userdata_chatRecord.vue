...
<template>
  <div id="store" class="shadow_container">
    ...
    <el-form ref="find_form" :model="find_form" label-width="80px">
      ...
      <el-form-item label="接收方ID" label-width="100px">
        <el-input v-model="find_form.data.userReceiveId" placeholder="请输入接收方ID"></el-input>
      </el-form-item>
      <el-form-item label="是否已读">
        <el-select v-model="find_form.data.isRead" placeholder="请选择是否已读">
          <el-option label="是" value="1"></el-option>
          <el-option label="否" value="0"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="聊天类型">
        <el-select v-model="find_form.data.ChatType" placeholder="请选择聊天类型">
          <el-option label="好友" value="User"></el-option>
    ...
    <el-table :data="store_list" border style="width: 100%"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="userReceiveId" label="接收方ID" width="120"></el-table-column>
      <el-table-column fixed prop="extend1" label="扩展字段" width="80"></el-table-column>
      <el-table-column fixed prop="ChatType" label="聊天类型" width="80"></el-table-column>
      <el-table-column prop="isRead" label="是否已读" width="120">
        <template slot-scope="scope">{{scope.row.isRead == '1'?'是':'否'}}</template></el-table-column>
    ...
    <el-dialog title="添加类别" :visible.sync="dialogFormVisibleOfAdd" width="40%">
      ...
      <el-form ref="add_form" :model="add_form" label-width="120px">
        ...
        <el-form-item label="接收方ID">
          <el-input v-model="add_form.userReceiveId" placeholder="请输入接收方ID" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="是否已读">          
        <el-select v-model="add_form.isRead" placeholder="请选择是否已读">
          <el-option label="是" value="1"></el-option>
          <el-option label="否" value="0"></el-option>
        </el-select>
        </el-form-item>
        <el-form-item label="聊天类型">              
        <el-select v-model="add_form.ChatType" placeholder="请选择聊天类型">
          <el-option label="好友" value="User"></el-option>
          <el-option label="群聊" value="Group"></el-option>
        </el-select>
        </el-form-item>
    ...
    <el-dialog title="修改类别" :visible.sync="dialogFormVisibleOfEdit" width="40%">
      ...
      <el-form ref="edit_form" :model="edit_form" label-width="120px">
        <el-form-item label="接收方ID">
          <el-input v-model="edit_form.userReceiveId" placeholder="请输入接收方ID" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="是否已读">          
        <el-select v-model="edit_form.isRead" placeholder="请选择是否已读">
          <el-option label="是" value="1"></el-option>
          <el-option label="否" value="0"></el-option>
        </el-select>
        </el-form-item>
        <el-form-item label="聊天类型">              
        <el-select v-model="edit_form.ChatType" placeholder="请选择聊天类型">
          <el-option label="好友" value="User"></el-option>
          <el-option label="群聊" value="Group"></el-option>
        </el-select>
        </el-form-item>
...
export default {
  ...
  mounted() {
    ...
    getDataList("u1", "Chatrecord", get_form, "store_list", this);
  },
  methods: {
    queryData() {
    ...
    user_add() {
      ...
      putcreate("u1", "Chatrecord",add_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    // 修改当前
    user_update() {
      patchedit("u1", "Chatrecord",this.edit_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    user_delete(val) {
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deleteuser("u1", "Chatrecord",del).then((res) => {
          console.log(res);
          if (res != null) {
    ...
    delete_list(){
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deletelist("u1", "Chatrecord",dellist).then((res) => {
          console.log(res);
          if (res != null) {
            this.$message.success("删除成功");
            this.resetForm();
