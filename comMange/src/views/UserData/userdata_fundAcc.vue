...
<template>
  <div id="store" class="shadow_container">
    ...
    <el-table :data="store_list" border style="width: 100%"  @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="goldCoinNum" label="金币数量" width="120"></el-table-column>
    ...
    <el-dialog title="添加类别" :visible.sync="dialogFormVisibleOfAdd" width="40%">
      ...
      <el-form ref="add_form" :model="add_form" label-width="120px">
        ...
        <el-form-item label="金币数量">
          <el-input v-model="add_form.goldCoinNum" placeholder="请输入金币数量" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="修改类别" :visible.sync="dialogFormVisibleOfEdit" width="40%">
      ...
      <el-form ref="edit_form" :model="edit_form" label-width="120px">
        <el-form-item label="金币数量">
          <el-input v-model="edit_form.goldCoinNum" placeholder="请输入金币数量" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  mounted() {
    ...
    getDataList("u1", "Fundacc", get_form, "store_list", this);
  },
  methods: {
    queryData() {
    ...
    user_add() {
      ...
      putcreate("u1", "Fundacc",add_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    // 修改当前
    user_update() {
      patchedit("u1", "Fundacc",this.edit_form).then((res) => {
        console.log(res);
        if (res != null) {
    ...
    user_delete(val) {
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deleteuser("u1", "Fundacc",del).then((res) => {
          console.log(res);
          if (res != null) {
    ...
    delete_list(){
      this.$confirm("确定要删除吗(｀・ω・´)", "提示", {
        ...
        deletelist("u1", "Fundacc",dellist).then((res) => {
          console.log(res);
          if (res != null) {
            this.$message.success("删除成功");
            this.resetForm();
